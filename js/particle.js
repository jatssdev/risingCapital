...
// Manually manipulate vertices with BufferGeometry:
var positionAttribute = box.geometry.attributes.position;
for (var j = 0; j < positionAttribute.count; j++) {
    var vertex = new THREE.Vector3().fromBufferAttribute(positionAttribute, j);
    vertex.x += -10 + Math.random() * 20;
    vertex.y += -10 + Math.random() * 20;
    vertex.z += -10 + Math.random() * 20;
